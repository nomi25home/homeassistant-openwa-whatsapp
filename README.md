...
## Recommended Backend Add-on

The recommended backend for this integration is the companion Home Assistant add-on:

https://github.com/nomi25home/homeassistant-openwa-addon
...
- QR setup and helper/status UI on port `2786`
- Persistent session storage
- Automatic session creation/startup handling
...
The recommended setup is the companion Home Assistant add-on:

https://github.com/nomi25home/homeassistant-openwa-addon

When using the add-on, configure this integration with:

| Field | Example |
|---|---|
| OpenWA Base URL | `http://HOME_ASSISTANT_HOST:2785` |
| OpenWA API Key | Same value as the add-on `openwa_api_key` |
| OpenWA Session ID | Session ID generated by the add-on |
...
- A valid WhatsApp chat ID, such as `15551234567@c.us`.

## Alternative: Deploying OpenWA without the Add-on
...
If you use the companion add-on, the session ID is shown in the add-on logs after startup and can also be checked through the sessions endpoint.

You can also use the OpenWA sessions endpoint:
